...
class GameLogic:
    ...
    def __init__(self,col,row,k,mode,debug):
        self.col = col
        self.row = row
        self.k = k
        self.mode = mode
    ...
    def gameloop(self,fh=None):
        ...
        winPlayer = 0
        move = Move([])
        board = Board(self.col,self.row,self.k)
    ...
    def TournamentInterface(self):
        ai = StudentAI(self.col,self.row,self.k)
        while True:
    ...
    def Run(self,fh=None,**kwargs):
        if self.mode == 'n' or self.mode == 'network' :
            if kwargs['mode'] == 'host':
                self.ai_list.append(
                    IOAI(self.col, self.row, self.k, ai_path=kwargs['ai_path'], time=kwargs['time']))
                self.ai_list.append(
                    NetworkAI(self.col, self.row, self.k, mode=kwargs['mode'], info=kwargs['info']))
            ...
            if kwargs['order'] == '1':
                self.ai_list.append(
                    ManualAI(self.col, self.row, self.k))
                self.ai_list.append(
            ...
            self.ai_list.append(
                IOAI(self.col, self.row, self.k, ai_path=kwargs['ai_path_1'], time=kwargs['time']))
            self.ai_list.append(
                IOAI(self.col, self.row, self.k, ai_path=kwargs['ai_path_2'], time=kwargs['time']))
